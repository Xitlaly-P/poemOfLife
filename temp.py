...
class board:
    def __init__(self):
        ...
        for i in range(0, 10):
            ...
            for j in range(0, 10):
                ...
                # Randomly decide if the tile should be a wildcard (10% chance)
                is_wildcard = random.randint(1, 100) <= 10
                
                if is_wildcard:
                    temp.append((x, y, count, "Wildcard"))
                else:
    ...
    def draw(self):
        ...
        for i in self.boardarr:
            for j in i:
                if j[3] == "Wildcard":
                    colorb = (255, 255, 0)  # Yellow color for wildcard tiles
                elif int(j[2]) % 2 == 0:
                    colorb = boardclr
                else:
                    colorb = boardclr2
                ...
                if j[3] == "Wildcard":
                    display_word = j[3]  # Display wildcard type
                elif len(j[3]) > 8:
...
class player:
        ...
        for x in self.barr:
            for y in x:
                if self.val == y[2]:
                    ...
                    self.ypos = y[1]
                    if y[3] == "Wildcard":
                        wildcard_type = random.choice(["Choose", "Remove"])
                        match wildcard_type:
                            case "Choose":
                                self.choose_word_from_tiles()
                            case "Remove":
                                self.collected_words.pop()
                    else:
    ...
    def choose_word_from_tiles(self):
        chosen_word = input("Enter the word you want to choose: ")
        self.collected_words.append(chosen_word)
        print(f"Appended chosen word: {chosen_word}")
